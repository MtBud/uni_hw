 ...
 * @return
 */
vector< set<Symbol> > CYK_step1( const Grammar& grammar, const Word& word ){
    vector< set<Symbol> > outVector;
    map< Symbol, set<Symbol> > terminalMap;

    // make map of what terminals are generated by what non-terminals
    for( const auto& i : grammar.m_Terminals ){
        terminalMap.insert( pair< Symbol, set<Symbol> >( i, set<Symbol>() ) );
        for( const auto& j : grammar.m_Rules ){
            if( j.second[0] == i ){
                terminalMap.at(i).insert( j.first );
            }
        }
    }
...
bool CYK( const Grammar& grammar, const Word& word ){
    map< int, vector< set<Symbol> > > columns;
    // add first column
    columns.insert( pair<int, vector< set<Symbol> > >(1, CYK_step1( grammar, word ) ) );

    // control print of the column
    for( const auto& i : columns[1] ){
        for( const auto& j : i )
            cout << j << " ";
        cout << endl;
    }
